...
# Jinja tweaks
app.jinja_options['trim_blocks'] = app.jinja_options['lstrip_blocks'] = True
...
def render_markdown(randomgarbage):
    ...
    if len(matches) == 1:
        ...
        filename = matches[0]
        filename = '/'.join([app.config['MARKDOWN_DATA_DIR'],filename+'.md'])
        ...
        return render_template('index.html'), 404
    return render_template('renderrrrrrr.html', document_name=filename, content=content)
